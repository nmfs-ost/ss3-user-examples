...
1 #_screen_display:  0 (minimal); 1 (one line per iter); 2 (each logL)
...
#
1 #_N_bootstraps:  Number of datafiles to produce:  0 turns off all *.ss_new; 1st is data_echo.ss_new, 2nd is data_expval.ss, 3rd and higher are data_boot_**N.ss,
...
1969 #_minyr_sdreport:  min year for sdreport outputs (-1 for styr); #_1969
2011 #_maxyr_sdreport:  max year for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2011
...
0.4 #_depl_denom_frac:  fraction (X) for Depletion denominator (e.g. 0.4)
1 #_SPR_basis:  0 (skip); 1 (1-SPR)/(1-SPR_tgt); 2 (1-SPR)/(1-SPR_MSY); 3 (1-SPR)/(1-SPR_Btarget); 4 (1-SPR); 5 (SPR)
4 # F_std_units:  0 (skip); 1 (exploitation(Bio)); 2 (exploitation(Num)); 3 (sum(Apical_F's)); 4 (mean F for range of ages (numbers weighted)); 5 (unweighted mean F for range of ages)
 20 23 # min and max age over which mean F will be calculated, with F=Z-M
1 # F_std_basis: 0=no scaling; 1 (F/Fspr); 2 (F/Fmsy); 3 (F/Fbtgt); where F means annual F_std, Fmsy means F_std@msy; values >=11 invoke N multiyr using 10s and 100s digit; append .1 to invoke log(ratio)
0.08 #_MCMC_output_detail: integer part (0=default; 1=adds obj func components; 2= +write_report_for_each_mceval); and decimal part (added to SR_LN(R0) on first call to mcmc)
0 #_deprecated:  ALK tolerance ***disabled in code
-1 #_seed:  random number seed for bootstrap data (-1 to use long(time) as seed): # 1757597700
