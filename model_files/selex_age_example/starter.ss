...
#C file write time: 2013-03-10 21:29:15
selex_age_example_data.ss #_datfile
selex_age_example_control.ss #_ctlfile
...
1 #_parmtrace:  write parm values to ParmTrace.sso:  0 (no); 1 (good_iter,active_parms); 2 (good,all); 3 (every,all); 4 (every,active)
...
-1 #_minyr_sdreport:  min year for sdreport outputs (-1 for styr); #_1999
-2 #_maxyr_sdreport:  max year for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2013
...
-1 #_seed:  random number seed for bootstrap data (-1 to use long(time) as seed): # 1758551506
